...
public class IndexRepository {
    ...
    public List<Integer> getNextPendingBatch(int batchSize) {
        String sql =
            "SELECT page_id FROM indexing_queue WHERE status='pending' LIMIT ?";
    ...
    public void addIndex(int pageId, Map<String, Integer> freqs) {
        Map<String, Integer> allTerms = addTerms(
            new ArrayList<>(freqs.keySet())
        );
        ...
        String markIndexedSql =
            "UPDATE indexing_queue SET status='indexed' WHERE page_id=?";

        try (
            Connection conn = Database.getConnection();
            PreparedStatement pstmt = conn.prepareStatement(sql);
            PreparedStatement markIndexedPstmt = conn.prepareStatement(
                markIndexedSql
            )
        ) {
            ...
            Integer[] termIds = entries
                .stream()
                .map(e -> allTerms.get(e.getKey()))
                .toArray(Integer[]::new);
            Integer[] termFreqs = entries
                .stream()
